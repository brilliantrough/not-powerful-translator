...
import pytesseract
from utils.google_trans import Google
...
def ocr_process(image_name: str, engine: str = "google") -> tuple:
    ...
    for block in block_nums:
        ...
        for i in range(len(data['text'])):
            if data['block_num'][i] == block and data['text'][i] != ' ' and data['text'][i] is not None:
    ...
    i = 0
    for block in block_nums:
        block_data = {k: [v[j] for j in range(len(data['text'])) if data['block_num'][j] == block] for k, v in data.items()}
        if len(block_data['text']) == 0:
            print("asdssddfsf")
            continue
        i += 1
